...
from src.zas.core.helpers import _get, _paginate_search, _put, mcp
...
def ticket_add_internal_note(ticket_id: int, body: str):
	...
	try:
		text = (body or "").strip()
		if not text:
			return {"ok": False, "error": "De body van de interne opmerking mag niet leeg zijn."}
	...
	try:
		...
		for t in tickets:
			...
			try:
				...
				public_comments = [
					c.get("body", "") for c in raw_comments if c.get("public", False)
				]
			except Exception:
				public_comments = []
			summarized = ""
			...
			time.sleep(0.1)
		lines = [
			f"- Ticket {b['id']} :: '{b['subject']}' :: Mogelijk probleem: {b['problem_summary']}"
			for b in bundle
		]
		combined_text = (
...
@mcp.tool
def tickets_analyze(
	query: str, limit: int = 200, enrich_with_metrics: bool = True, metrics_sample: int = 50
):
	"""[Ticket-Agent]
	...
	try:
		...
		insights = {
			...
			"top_tags": sorted(tag_counts.items(), key=lambda x: x[1], reverse=True)[:25],
			"top_assignees_by_ticket_count": sorted(
				by_assignee.items(), key=lambda x: x[1], reverse=True
			)[:25],
		}
		...
		if metrics_rows:
			...
			reopen_counts = [
				r.get("reopens") for r in metrics_rows if isinstance(r.get("reopens"), (int, float))
			]
			reply_counts = [
				r.get("replies") for r in metrics_rows if isinstance(r.get("replies"), (int, float))
			]
			...
			def _summary(vs):
				...
				n = len(vs_sorted)
				p50 = (
					vs_sorted[n // 2] if n % 2 else (vs_sorted[n // 2 - 1] + vs_sorted[n // 2]) / 2
				)
